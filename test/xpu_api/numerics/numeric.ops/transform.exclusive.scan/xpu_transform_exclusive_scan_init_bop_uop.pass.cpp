// -*- C++ -*-
//===-- xpu_transform_exclusive_scan_init_bop_uop.pass.cpp
//--------------------------------------------===//
//
    ...
    sycl::range<1> numOfItems1{5};
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<Iter>([=]() {
                transform_exclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out1[0], 0, oneapi::dpl::plus<>(), add_one{});
                transform_exclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out2[0], 0, oneapi::dpl::multiplies<>(),
                                         add_one{});
                transform_exclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out3[0], 0, oneapi::dpl::plus<>(),
                                         oneapi::dpl::negate<>());
                transform_exclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out4[0], 0, oneapi::dpl::multiplies<>(),
                                         oneapi::dpl::negate<>());
            });
        });
    }
