// -*- C++ -*-
//===-- xpu_transform_inclusive_scan_bop_uop_init.pass.cpp
//--------------------------------------------===//
//
...
test(sycl::queue& deviceQueue)
{
    ...
    sycl::range<1> numOfItems1{5};
    {
        ...
        sycl::buffer<int, 1> buffer4(output3, numOfItems1);
        sycl::buffer<int, 1> buffer5(output4, numOfItems1);
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<Iter>([=]() {
                transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out1[0], oneapi::dpl::plus<>(), add_one{}, 0);
                transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out2[0], oneapi::dpl::multiplies<>(),
                                         add_one{}, 0);
                transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out3[0], oneapi::dpl::plus<>(),
                                         oneapi::dpl::negate<>(), 0);
                transform_inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out4[0], oneapi::dpl::multiplies<>(),
                                         oneapi::dpl::negate<>(), 0);
            });
        });
    }
    const int ref1[5] = {2, 6, 12, 20, 30};
    const int ref2[5] = {0, 0, 0, 0, 0};
    const int ref3[5] = {-1, -4, -9, -16, -25};
