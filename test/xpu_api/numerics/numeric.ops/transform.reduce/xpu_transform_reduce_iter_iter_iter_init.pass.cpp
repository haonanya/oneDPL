// -*- C++ -*-
//===-- xpu_transform_reduce_iter_iter_iter_init.pass.cpp
//--------------------------------------------===//
//
    ...
    sycl::range<1> numOfItems2{8};
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<KernelName<Iter1, Iter2>>([=]() {
                out[0] = transform_reduce(Iter1(&in1[0]), Iter1(&in1[0]), Iter2(&in2[0]), 0);
                ...
                out[3] = transform_reduce(Iter2(&in2[0]), Iter2(&in2[0] + 1), Iter1(&in1[0]), 2);
                out[4] = transform_reduce(Iter1(&in1[0]), Iter1(&in1[0] + 2), Iter2(&in2[0]), 0);
                out[5] = transform_reduce(Iter2(&in2[0]), Iter2(&in2[0] + 2), Iter1(&in1[0]), 3);
                out[6] = transform_reduce(Iter1(&in1[0]), Iter1(&in1[0] + 6), Iter2(&in2[0]), 0);
                out[7] = transform_reduce(Iter2(&in2[0]), Iter2(&in2[0] + 6), Iter1(&in1[0]), 4);
            });
        });
    }
