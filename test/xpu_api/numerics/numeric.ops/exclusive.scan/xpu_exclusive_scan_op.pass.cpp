...
test(sycl::queue& deviceQueue)
{
    ...
    sycl::range<1> numOfItems1{5};
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<KernelTest<Iter1>>([=]() {
                // Not in place
                oneapi::dpl::exclusive_scan(Iter1(&in[0]), Iter1(&in[0] + 5), &out1[0], 0, oneapi::dpl::plus<>());
                // In place
                oneapi::dpl::exclusive_scan(Iter1(&out2[0]), Iter1(&out2[0] + 5), &out2[0], 0, oneapi::dpl::plus<>());
            });
        });
    }
