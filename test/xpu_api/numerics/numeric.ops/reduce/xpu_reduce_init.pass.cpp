...

    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<Iter>([=]() {
                out[0] = reduce(Iter(&in[0]), Iter(&in[0]), 0);
                out[1] = reduce(Iter(&in[0]), Iter(&in[0]), 1);
                out[2] = reduce(Iter(&in[0]), Iter(&in[0] + 1), 0);
                out[3] = reduce(Iter(&in[0]), Iter(&in[0] + 1), 2);
                out[4] = reduce(Iter(&in[0]), Iter(&in[0] + 2), 0);
                out[5] = reduce(Iter(&in[0]), Iter(&in[0] + 2), 3);
                out[6] = reduce(Iter(&in[0]), Iter(&in[0] + 6), 0);
                out[7] = reduce(Iter(&in[0]), Iter(&in[0] + 6), 4);
            });
        });
    }
