// -*- C++ -*-
//===-- xpu_inclusive_scan_op_init.pass.cpp
//--------------------------------------------===//
//
    ...
    sycl::range<1> numOfItems1{5};
    {
        ...
        deviceQueue.submit([&](sycl::handler& cgh) {
            ...
            cgh.single_task<KernelTest<Iter>>([=]() {
                // Not in place
                oneapi::dpl::inclusive_scan(Iter(&in[0]), Iter(&in[0] + 5), &out1[0], oneapi::dpl::plus<>(), 0);
                // In place
                oneapi::dpl::inclusive_scan(Iter(&out2[0]), Iter(&out2[0] + 5), &out2[0], oneapi::dpl::plus<>(), 0);
            });
        });
    }
